...
export default function DoctorChatbotPage() {
  ...
  useEffect(() => {
    ...
    const savedMessages = localStorage.getItem('doctor_chatbot_messages')
    if (savedMessages) {
      ...
      const initialMessage = {
        id: '1',
        type: 'bot' as const,
        content: 'Hello Doctor! I\'m your HealthBridge AI assistant. I can help with patient information, scheduling, and medical queries.',
        timestamp: new Date(),
      }
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      localStorage.setItem('doctor_chatbot_messages', JSON.stringify(messages))
    }
  }, [messages])
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      response = await fetch(`${process.env.NEXT_PUBLIC_AI_API_URL || 'https://health-bridge-app-3.onrender.com'}/ai/chat`, {
        ...
        body: JSON.stringify({
          message: inputValue,
          context: `You are a healthcare AI assistant for a ${userRole || 'doctor'}. Provide professional medical insights, patient management advice, and clinical decision support while maintaining medical ethics and standards.`
        })
      });
      ...
      <div className="space-y-4 sm:space-y-6 w-full">
        <div className="flex items-center space-x-2 sm:space-x-3 w-full">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 truncate">Chat with your medical AI assistant</p>
          </div>
        </div>
        ...
        <div className="md:h-[70vh] bg-white dark:bg-gray-800 rounded-lg shadow flex flex-col w-full max-w-full" style={{ height: 'calc(100vh - 160px)' }}>
          ...
          <div className="px-3 sm:px-6 py-2 sm:py-3 border-t border-gray-200 dark:border-gray-700 w-full overflow-x-hidden">
            <div className="flex flex-wrap gap-1 sm:gap-2 w-full">
              <button
                onClick={() => setInputValue('Show me today\'s patient schedule')}
              ...
              <button
                onClick={() => setInputValue('How many patients are in the queue?')}
                className="px-2 sm:px-3 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 flex-shrink-0"
              >
                Queue status
              </button>
              <button
                onClick={() => setInputValue('Find patient medical history')}
                className="px-2 sm:px-3 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 flex-shrink-0"
              >
                Patient history
              </button>
          ...
          <div className="p-3 sm:p-6 sm:pt-3 border-t border-gray-200 dark:border-gray-700 w-full">
            <form onSubmit={handleSendMessage} className="flex space-x-2 sm:space-x-3 w-full">
              <input
                ...
                placeholder="Ask about patients, schedules, or medical queries..."

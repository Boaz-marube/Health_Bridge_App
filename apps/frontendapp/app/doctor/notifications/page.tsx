...
import { Bell, CheckCircle, AlertTriangle, Info, Clock, Eye, EyeOff, Calendar, Pill, Heart, Users, Trash2 } from 'lucide-react'
...
export default function DoctorNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (userData) {
        ...
        const data = await notificationService.getNotifications(user.id, 'doctor')
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      if (userData) {
        const user = JSON.parse(userData)
        await notificationService.markAllAsRead(user.id, 'doctor')
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'patient_update': return <Users className="h-5 w-5 text-green-500" />
